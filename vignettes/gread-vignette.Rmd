...
* By default, the argument `tidy_cols` is `TRUE`. This argument prepares the data 
...
## The `tidy_cols()` function

In case the default tidying removes columns that you'd like to retain, you 
can set `tidy_cols=FALSE` first while loading the files using the function(s) 
shown above, and then use the function `tidy_cols()` to remove just the 
columns you don't need using the `remove_cols` argument.
...
tidy_cols(gtf, remove_cols = c("tss_id", "attributes"))[]
```
